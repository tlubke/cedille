...
no-parens {_} {TERM} _ (Delta _ _ _ _) right = tt
...
no-parens {TERM} {_} (Delta pi mT t ot) p lr = ff
...
no-parens {KIND} {_} (KndPi pi pi' x Tk k) p lr = (is-arrow p || is-abs p) && not-left lr
no-parens {KIND} {_} (KndTpArrow T k) p lr = (is-arrow p || is-abs p) && not-left lr
...
term-to-stringh (Delta pi mT t ot) = strBreak' ((0 , strAdd "δ") :: (optType-to-string 2 nothing mT) ++ (2 , strAdd "-") :: [ 2 , to-stringr t ] ++ optTerm-to-string ot "{ " " }")
